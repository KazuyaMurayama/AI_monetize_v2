# AI_monetize v2 — Claude Code 運用ルール

AI 関連事業のマネタイズ戦略立案・実行支援リポジトリ（v2）。
...
3. **`FILE_INDEX.md`** — ファイル一覧インデックス（存在する場合）
...
| リポ | 役割 |
|---|---|
| [KazuyaMurayama/AI-Transformation-Architect-monetize](https://github.com/KazuyaMurayama/AI-Transformation-Architect-monetize) | AI 変革アーキテクトのマネタイズパイプライン（v2 の上位概念） |
| [KazuyaMurayama/NASDAQ-strategy-monetize](https://github.com/KazuyaMurayama/NASDAQ-strategy-monetize) | NASDAQ 戦略のマネタイズ事例 |
| [KazuyaMurayama/freelance-compass](https://github.com/KazuyaMurayama/freelance-compass) | フリーランス案件獲得 |
...
| 成果物 | 説明 | リンク |
|---|---|---|
| file.md | 1行説明 | [開く](https://github.com/KazuyaMurayama/AI_monetize_v2/blob/main/path/to/file.md) |

**厳守事項**
1. 必ず Markdownリンク `[表示名](URL)` 形式。plain text URL 禁止。
2. `/blob/<実ブランチ>/<実パス>` 形式。リポトップ URL 禁止。
3. **報告前に URL 存在確認**: `gh api repos/KazuyaMurayama/AI_monetize_v2/contents/PATH?ref=main` でステータス200確認。
...
### ファイル名
- 基本形 `<TOPIC>_YYYYMMDD.md`（**サフィックス・ハイフンなし**）。例: `MONETIZE_STRATEGY_20260603.md`
- 同日中の追加更新は `-v2`、`-v3`（例: `MONETIZE_STRATEGY_20260603-v2.md`）。
- 日付が変わったら v サフィックスをリセット（例: 翌日1回目 `MONETIZE_STRATEGY_20260604.md`）。
...
### 対象外（日付サフィックスを入れない）
README / CLAUDE.md / FILE_INDEX / file_index / tasks.md / CHANGELOG / LICENSE / SPEC.md / `CURRENT_*.md` / パイプライン自動生成ファイル（`outputs/*.md` 等）。

### 旧形式（廃止・新規禁止）
- ❌ `2026-06-03_<TOPIC>.md`（プレフィックス・ハイフン） / ❌ `<TOPIC>_2026-06-03.md`（サフィックス・ハイフン）
...
## 11. 次アクション提案（プロジェクトオーナーへ）

- **README.md を整備する**: プロジェクト目的・主要機能・セットアップ手順を明記する。
- v1 が存在するなら `AI_monetize_v1` リポへのリンクを README に追加する。
- v2 の差分・改善点を CHANGELOG.md または README で記述する。
